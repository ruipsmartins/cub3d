...
void	move_in_direction(t_player *player, t_game *game, float dx, float dy)
{
	...
	new_x = player->x + dx;
	new_y = player->y + dy;
	if (!is_colliding(game, new_x + PLAYER_RADIUS, new_y) && !is_colliding(game,
			new_x - PLAYER_RADIUS, new_y) && !is_colliding(game, new_x, new_y
			+ PLAYER_RADIUS) && !is_colliding(game, new_x, new_y
			- PLAYER_RADIUS))
	{
		player->x = new_x;
		player->y = new_y;
